...
*(対応: spec.md 5章 TAB譜データ仕様 / 7章 判定仕様 / 8章 スコア計算仕様 / 9章 スコア保存仕様)*
...
- YIN法を選んだ理由は spec.md 6章の通り。実装は `audio/pitch/yin.ts` に純粋関数として置き、
...
- v1は単音検出のみを対象とするため、YINの結果として最も確からしい基本周波数を1つだけ採用する
  (複数ピークの分離は行わない。spec.md 6章)

## 6. 判定エンジン

*(対応: spec.md 7章 判定仕様、7.1節の許容誤差テーブル)*

- ゲームクロックは `AudioContext.currentTime` を基準にする(`performance.now()` 単体より
  オーディオ処理との同期が取りやすいため)
- 各ノーツは `timeMs`(判定ラインに到達する予定時刻)を持つ。現在時刻との差分から画面上の
  x座標を計算し描画する(7章の通り、判定ラインは画面右側の固定位置)
- 判定はノーツごとに、判定ウィンドウ(選択中の `DifficultyPreset.window`)に基づき以下の順で行う
  ...
  3. spec.md 7.1節の通り、タイミング判定とピッチ判定のうち**厳しい方**を採用してランクを決定
...
- 判定結果 `JudgementResult` は都度スコア計算エンジン(7章)に渡す

## 7. スコア計算

*(対応: spec.md 8章 スコア計算仕様)*

spec.md 8章の式をそのまま実装する。
...
- `consecutivePerfect` は Perfect が連続した回数。Good/Miss を挟んだ時点で0にリセットする
  (spec.md 8章「コンボ倍率」の定義通り)
- 合計は四捨五入して `bestScore` と比較する
- ランク(S/A/B/C…)の閾値は spec.md 8章の通り未確定のため、`difficulty/rankTable.ts` に
...
*(対応: spec.md 9章 スコア保存仕様、10章 非機能要件)*

- `localStorage` に以下のキーで保存する(v1、サーバー同期なし)
  - `fretrush:scores` … `ScoreRecord[]`(曲ID×難易度IDごとに1件、更新時は上書き)
  - `fretrush:settings` … キャリブレーションのオフセット値、直近選択した判定プリセット等
  - `fretrush:charts` … インポート済み譜面のメタデータ一覧(実ファイルはIndexedDB化を
    将来検討。v1はサイズの小さい譜面を主対象とするためlocalStorageで許容する)
- 記録単位は spec.md 9章の通り「曲 × 判定の厳しさ」。詳細設定でプリセットから外れた場合の
  扱い(専用レコード化するか、最も近いプリセットに丸めるか)は、9章に明記の通り実装時に決定する
...
*(対応: spec.md 11章 v1スコープ外)*

以下は本設計書でも意図的に設計を作り込まない。将来必要になった時点で章を追加する。

- 和音判定用のポリフォニック音高推定(spec.md 11章)
- mp3等からのTAB譜自動生成(spec.md 11章)
- 変則チューニング対応(spec.md 11章)
- サーバー同期・オンラインランキング(spec.md 11章)
